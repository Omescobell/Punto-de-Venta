...
### 32. Inventory Valuation & Financial Metrics
...
### 33 . Product Contribution to Sales

Calculates the percentage of total sales generated by a specific product within a given date range. If no dates are provided, it defaults to the last 30 days.

*   **Endpoint:** `/api/analytics/product-contribution/`, examples: Default 30 days `/api/analytics/product-contribution/?product_identifier=COCA001`, Custom period `/api/analytics/product-contribution/?product_identifier=COCA001&start_date=2023-01-01&end_date=2023-12-31`
...
*   **`product_identifier` (String) Required:** The exact `name` or barcode (`SKU`) of the product.

*   **`start_date` (String - YYYY-MM-DD) Optional:** The start date of the period.

*   **`end_date` (String - YYYY-MM-DD) Optional:** The end date of the period.
...
```json
{
  ...
  "contribution_metrics": {
    ...
    "total_product_sales": 100.00,
    "total_general_sales": 200.00
  }
}
```
**Scenario A: Missing Product Identifier (400 Bad Request)**
Occurs when the endpoint is called without the mandatory `product_identifier` query parameter.
```json
{
  "error": "Product identifier is required."
}
```
**Scenario B: Product Not Found (404 Not Found)**
Occurs when the provided `product_identifier` does not match any existing `SKU` or product `name` in the database.
```json
{
  "error": "Product not found."
}
```
**Scenario C: Invalid Date Format (400 Bad Request)**
Occurs when custom dates are provided, but they do not follow the standard ISO 8601 format.
```json
{
  "error": "Invalid date format. Use YYYY-MM-DD."
}
```
**Scenario D: No Sales in Period / Zero Division Prevention (404 Not Found)**
Occurs when the product exists, but there were no general sales recorded within the specified time frame, resulting in a General Total Sale of $0.00 (which prevents a zero-division error).
```json
{
  "error": "No sales recorded in the specified period."
}
```
